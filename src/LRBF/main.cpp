...
int main() {
  ...
  for (int i = 0; i < MESH.Nbou; i++) {
    if (i < MESH.Nsurf) {
      MODEL.SetNodeBCType(
          i, GMMMFSSLOSHING<GMMRECTANGLE,
                            GMMMQBasis2D>::BoundaryType::FreeSurface);
    }

    else if (i >= MESH.Nsurf) {
      MODEL.SetNodeBCType(
          i, GMMMFSSLOSHING<GMMRECTANGLE, GMMMQBasis2D>::BoundaryType::Wall);
    }
  }

  // Start simulation
  MODEL.Start(sloshing_amp, sloshing_fre, StepN, dt, SaveStepN, PrintStepN,
              TOLERANCEPHI, TOLERANCEETA, SaveFolderName);

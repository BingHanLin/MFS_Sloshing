...
template <typename MeshType, typename RBFBasisType> class GMMMFSSLOSHING {

public:
  enum class BoundaryType { Wall, FreeSurface };
  ...
  vector<double> UpdateDFSBC(const vector<double> AccTank,
                             const double AccG) const;
  ...
  void Start(double sloshing_amp, double sloshing_fre, int StepN, double ddt,
             int SaveStepN, int PrintStepN, double TOLERANCEPHI,
             double TOLERANCEETA, string SaveFolderName);
...
GMMMFSSLOSHING<MeshType, RBFBasisType>::GMMMFSSLOSHING(MeshType &MMESH,
                                                       RBFBasisType &RRBFBasis)
...
template <typename MeshType, typename RBFBasisType>
void GMMMFSSLOSHING<MeshType, RBFBasisType>::assembly() {
...
template <typename MeshType, typename RBFBasisType>
vector<double> GMMMFSSLOSHING<MeshType, RBFBasisType>::UpdateDFSBC(
    const vector<double> AccTank, const double AccG) const {
  ...
  for (int i = 0; i < Nsurf; i++) {
    ...
    Phi_temp[i] =
        (Phi_n[i] +
         dt * (-0.5 * (pow((d_Phi_n[i] + d_Phi_n1[i]) / 2, 2) +
...
template <typename MeshType, typename RBFBasisType>
vector<double> GMMMFSSLOSHING<MeshType, RBFBasisType>::UpdateKFSBC() const {
  vector<double> Eta_temp(Nsurf);

  for (int i = 0; i < Nsurf; i++) {

    Eta_temp[i] =
        (Eta_n[i] + dt * (+((d_Phi_n[i + Nbou] + d_Phi_n1[i + Nbou]) / 2) -
                          ((d_Phi_n[i] + d_Phi_n1[i]) / 2) *
                              ((d_Eta_n[i] + d_Eta_n1[i]) / 2)));
  }
...
template <typename MeshType, typename RBFBasisType>
vector<double> GMMMFSSLOSHING<MeshType, RBFBasisType>::SolveDerPhi(
    const vector<double> &Phi_temp) {
  ...
  gmm::mult(Dx_PhiMatrix, Alpah_temp,
            gmm::sub_vector(d_Phi_temp, gmm::sub_interval(0, Nbou)));
  gmm::mult(Dy_PhiMatrix, Alpah_temp,
            gmm::sub_vector(d_Phi_temp, gmm::sub_interval(Nbou, Nbou)));

  return d_Phi_temp;
}

template <typename MeshType, typename RBFBasisType>
vector<double> GMMMFSSLOSHING<MeshType, RBFBasisType>::SolveDerEta(
    const vector<double> &Eta_temp) {
...
template <typename MeshType, typename RBFBasisType>
void GMMMFSSLOSHING<MeshType, RBFBasisType>::Start(
    double sloshing_amp, double sloshing_fre, int StepN, double ddt,
    int SaveStepN, int PrintStepN, double TOLERANCEPHI, double TOLERANCEETA,
  ...
  for (int Istep = 1; Istep <= StepN; Istep++) {
    AccTank[0] = -sloshing_amp * sloshing_fre * sloshing_fre *
                 sin(sloshing_fre * dt * Istep);
    AccTank[1] = 0.0;
    ...
    for (int CNIter = 1; CNIter <= CNIterMax; CNIter++) {
      ...
      gmm::copy(UpdateDFSBC(AccTank, AccG),
                gmm::sub_vector(Phi_n1, gmm::sub_interval(0, Nsurf)));
...
template <typename MeshType, typename RBFBasisType>
void GMMMFSSLOSHING<MeshType, RBFBasisType>::SetNodeBCType(
    int i, BoundaryType bctype) {
  NodeBCType[i] = bctype;
}

template <typename MeshType, typename RBFBasisType>
void GMMMFSSLOSHING<MeshType, RBFBasisType>::CheckNodeBCType() {
...
template <typename MeshType, typename RBFBasisType>
void GMMMFSSLOSHING<MeshType, RBFBasisType>::SaveData(string FolderName,
                                                      double SaveTime) {
  ...
  char *charFolderPath = new char[FolderPath.size() + 1];
  strcpy(charFolderPath, FolderPath.c_str());

  mkdir(charFolderPath);

  delete charFolderPath;

  // Save Boundadry Nodes
  ofstream fout(FilePath);

  for (int i = 0; i < Nbou; i++) {
    fout << BouNodes[i][0] << ", " << BouNodes[i][1] << ", " << SouNodes[i][0]
         << ", " << SouNodes[i][1] << endl;
  }
...
template <typename MeshType, typename RBFBasisType>
void GMMMFSSLOSHING<MeshType, RBFBasisType>::DeleteOriginData(
    string FolderName) {
  ...
  char *charDeleteString = new char[DeleteString.size() + 1];
  strcpy(charDeleteString, DeleteString.c_str());

  system(charDeleteString);

  delete charDeleteString;
}

...
class GMMRECTANGLE
{
    ...
    public:
        ...
        GMMRECTANGLE (int mm, int nn, double LLength, double HHeight, 
                      double LLengthCenter = 0.0, double HHeightCenter = 0.0, double GGamma = 1.5 );
        ~GMMRECTANGLE() {};
...
GMMRECTANGLE::GMMRECTANGLE(int mm, int nn, double LLength, double HHeight, 
                           double LLengthCenter, double HHeightCenter, double GGamma)
{
    LengthNode = mm;
    HeightNode = nn;
    Length = LLength;
...
void GMMRECTANGLE::CreateBoundaryNode()
{
    ...
    for (int i = 0; i < Nbou; i++)
    {
        ...
        if(i < LengthNode)
        {
            temp[0]=(double)(i)*Length/(double)(LengthNode)+.5*Length/(double)(LengthNode);
            temp[1]=Height;
        }
        else if(i < LengthNode+HeightNode)
        {
            temp[0]=Length;
            temp[1]=Height-.5*Height/(double)(HeightNode)-(double)(i-LengthNode)*Height/(double)(HeightNode);
        }
        else if(i < LengthNode+HeightNode+LengthNode)
        {
            temp[0]=Length-.5*Length/(double)(LengthNode)-(double)(i-LengthNode-HeightNode)*Length/(double)(LengthNode);
            temp[1]=0.;
        }
        else
        {
            temp[0]=0.;
            temp[1]=(double)(i-LengthNode-HeightNode-LengthNode)*Height/(double)(HeightNode)+.5*Height/(double)(HeightNode);
        }
...
void GMMRECTANGLE::UpateBoundaryNode(vector<double> EtaDiff)
{
    ...
    for (int i = 0; i < Nbou; i++)
    {
        ...
        else if(i < LengthNode+HeightNode)
        {
            ...
            Height = BouNodes[LengthNode-1][1] - BouNodes[LengthNode + HeightNode][1];

            BouNodes[i][1] = ( 
                                 BouNodes[LengthNode + HeightNode][1] + 
                                Height-.5*Height/(double)(HeightNode)-(double)(i-LengthNode)*Height/(double)(HeightNode)
                            ); 
        ...
        else
        {
            ...
            BouNodes[i][1] = ( 
                                BouNodes[2*LengthNode + HeightNode-1][1] + 
                                (double)(i-2*LengthNode-HeightNode)*Height/(double)(HeightNode)+.5*Height/(double)(HeightNode)
                            ); 
        }
    }
